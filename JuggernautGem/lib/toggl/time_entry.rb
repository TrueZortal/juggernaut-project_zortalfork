    ...
    class TimeEntry
      ...
      def split(breakpoint:)
        ...
        if first_duration > 0
          ...
          puts '✂️  First part removed, because it\'s negligible'
        end
      ...
      def self.calculate_duration(scope_start:, scope_end:)
        value = ((DateTime.parse(scope_end).to_time - DateTime.parse(scope_start).to_time) * 1000).to_i
        puts "calculated duration #{value}"
        value
      end
